...
def make_inchikey_dict(mol_list):
    ...
    inchikey_dict = {}
    for mol in mol_list:
        inchikey = mol.GetProp(ms_constants.SDF_TAG_INCHIKEY)  # get pred-defined property values
        if inchikey not in inchikey_dict:
            inchikey_dict[inchikey] = [mol]
        else:
            inchikey_dict[inchikey].append(
                mol)  # one molecular have one and only one inchikey but one inchikey may yield  more one molecular
